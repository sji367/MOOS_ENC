...
IvPFunction *BHV_Waypoint::onRunState() 
{
  ...
  //--------------------------------------------------------------------//
  //--------------------------------------------------------------------//
  // Note the waypoint prior to possibly incrementing the waypoint. However,
  // If m_flag_update_WPT == true, we definitely updated the waypoint
  // but we have to handle this case explicitly. If the current index
  // is greater than 0 we can simply decrement it one. If it's equal
  // to 0, we either set the previous point to the last point in the
  // list, when cycle count is > 0 (i.e. we're looping repeatedly
  // through the points), or we set it to own-ship's position.
  double this_x = 0;
  double this_y = 0;
...
bool BHV_Waypoint::setNextWaypoint()
{
  ...
  //--------------------------------------------------------------------//
  string feedback_msg;
  // In this statement, we deal with the update waypoint flag. If the 
  // flag is set, we run setWaypoint instead of setNextWaypoint. The 
  // difference between the two functions is setWaypoint does not 
  // determine if the ASV has arrived at the waypoint because we 
  // have changed the desired waypoint. However, both functions return
  // a feedback message that will either be: empty_seglist, completed,
  // cycled, advanced, or in-transit
  if (m_flag_update_WTP == true)
    {
...
void BHV_Waypoint::postStatusReport()
{
  ...
  //--------------------------------------------------------------------//
  //--------------------------------------------------------------------//
  // Post information on the next waypoint to the MOOSDB
